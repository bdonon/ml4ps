...
def split_rng_like(rng, h2mg: H2MG) -> H2MG:
    ...
    rng_h2mg = h2mg*0 #empty_like(h2mg)
...
def uniform_like(rng, h2mg: H2MG) -> H2MG:
    r"""Draws a H2MG with same structure as provided `h2mg`, with features sampled from :math:`\mathcal{U}([0,1])`.
    ...
    """
    flat_h2mg, unravel = ravel_pytree(h2mg) #h2mg.flatten()
    x = jax.random.uniform(rng, flat_h2mg.shape)
    return unravel(x)#h2mg.unflatten_like(x)

def normal_like(rng, h2mg: H2MG) -> H2MG:
    r"""Draws a H2MG with same structure as provided `h2mg`, with features sampled from :math:`\mathcal{N}(0,I)`."""
    flat_h2mg, unravel = ravel_pytree(h2mg) #h2mg.flatten()
...
def categorical(rng, logits: H2MG, deterministic=False) -> H2MG:
    ...
    flat_logits, unravel = ravel_pytree(logits)#logits.flatten()
    if deterministic:
    ...
    res_action = unravel(flat_res_action)#logits.unflatten_like(flat_res_action)
    return res_action
...
def categorical_logprob(x_onehot: H2MG, logits: H2MG) -> float:
    ...
    """
    flat_onehot, _ = ravel_pytree(x_onehot)
    flat_onehot = jax.lax.stop_gradient(flat_onehot)
    flat_logits, _ = ravel_pytree(logits)
    flat_logprobs = jnp.nan_to_num(jax.nn.log_softmax(jnp.nan_to_num(flat_logits), nan=-jnp.inf), neginf=0.)
...
def categorical_per_feature(rng, logits:H2MG, deterministic=False) -> H2MG:
    ...
    if deterministic:
        ...
        return tree_map(partial(jnp.argmax, axis=-1), logits_clean)
    #r = map_to_features(jax.random.categorical, [rng_h2mg, logits_clean])
    r = tree_map(jax.random.categorical, rng_h2mg, logits_clean)
    return r + logits[:, 0] * 0
...
def categorical_per_feature_logprob(x_idx: H2MG, logits: H2MG) -> float:
    ...
    selected_logits = tree_map(lambda logits, x_i: jnp.take_along_axis(jnp.nan_to_num(logits, nan=0),
                                                                              jnp.expand_dims(x_i.astype(jnp.int32), 1),
                                                                              axis=-1), logits, x_idx)
